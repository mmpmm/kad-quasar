...
class QuasarRules {
  ...
  publish(request, response, next) {
    ...
    if (this.quasar.isSubscribedTo(topic)) {
      ...
      return async.each(neighbors, (contact, done) => {
    ...
    async.each(neighbors, (contact, done) => {
      this.quasar.pullFilterFrom(contact, (err, filter) => {
        if (!this._shouldRelayToContact(request, filter)) {
          contact = this.quasar._getRandomOverlayNeighbor();
        }
   ...
   * @private
   */
  _shouldRelayPublication(request, filter) {
    let negated = true;
