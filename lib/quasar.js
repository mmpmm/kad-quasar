...
Quasar.prototype.publish = function(topic, data) {
  ...
  var message = kad.Message({
    ...
    params: {
      ...
      contents: data,
      publishers: [nodeID],
      ttl: Date.now() + this._options.ttl
    }
  });
...
Quasar.prototype._handlePublish = function(params, callback) {
  ...
  if (this._bf.filters[0].has(params.topic)) {
    // If we are, the execute our subscription handler
    this._groups[params.topic](params.content);
...
Quasar.prototype._relayPublication = function(neighbors, params, callback) {
  ...
  async.filter(neighbors, function(contact, done) {
    self._getBloomFilterFromNeighbor(contact, function(err, atbf) {
      ...
      for (var i = 0; i < self._options.bf.depth; i++) {
...
Quasar.prototype._handleUpdate = function(params, callback) {
  ...
  if (!this._isValidBloomFilter(params.bloom)) {
    return callback(new Error('Invalid request received'));
  }

  params.bloom.filters.forEach(function(filter, index) {
    self._bf.filters[index].bitfield = filter.bitfield;
  });
...
Quasar.prototype._getBloomFilterFromNeighbor = function(contact, callback) {
  ...
  this._router._rpc.send(contact, message, function(err, message) {
    ...
    if (!message.result.bloom) {
      return callback(new Error('Invalid response received'));
    }

    var atbf = new BloomFilter(
      message.result.bloom.size,
      message.result.bloom.depth
    );

    message.result.bloom.filters.forEach(function(filter, index) {
      atbf.filters[index].bitfield = filter.bitfield;
    });
...
Quasar.prototype._updateNeighbor = function(contact) {
  ...
  this._router._rpc.send(contact, message);
};
...
Quasar.prototype._getRandomOverlayNeighbor = function(nodeID, topic) {
  var randIndex = kad.utils.getBucketIndex(nodeID, topic);
