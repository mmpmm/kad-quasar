...
Quasar.prototype._applyAttenuatedBloomFilterUpdate = function(atbf) {
  ...
  for (var f = 1; f < this._atbf.length; f++) {
    ...
    for (var b = 0; b < atbf[f - 1].bitfield.buffer.length; b++) {
      var local = this._atbf[f].bitfield.buffer;
      var remote = atbf[f - 1].bitfield.buffer;

      local[b] = local[b] | remote[b];
    }
  }
...
Quasar.prototype._relayPublication = function(neighbors, params, callback) {
  ...
  function _relayToRandomNeighbor(done) {
    ...
    self._sendPublish(randNeighbor, params, done || function() {});
  }
  ...
  if (self._options.randomRelay) {
    return async.times(kad.constants.ALPHA, _relayToRandomNeighbor);
  }
...
Quasar.prototype._handleSubscribe = function(params, callback) {
  callback(null, { filters: this._atbf.toString().split(',') });
};
...
Quasar.prototype._handleUpdate = function(params, callback) {
  this._applyAttenuatedBloomFilterUpdate(
    AttenuatedBloomFilter.from(params.filters)
  );
...
Quasar.prototype._updateNeighbor = function(contact) {
  ...
  var message = kad.Message({
    ...
    params: {
      filters: this._atbf.toString().split(','),
      contact: this._router._self
    }
  });
