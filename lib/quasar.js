...
Quasar.prototype._applyAttenuatedBloomFilterUpdate = function(atbf) {
  ...
  for (var f = 1; f < this._bf.depth; f++) {
    ...
    for (var b = 0; b < atbf.filters[f].bitfield.buffer.length; b++) {
      ...
      var remote = atbf.filters[f-1].bitfield.buffer;
...
Quasar.prototype.isSubscribedTo = function(topic) {
  return this._bf.filters[0].has(topic) && this._groups[topic];
};
...
Quasar.prototype.hasNeighborSubscribedTo = function(topic) {
  ...
  while (this._bf.filters[index]) {
    if (this._bf.filters[index].has(topic)) {
      return true;
    } else {
      index++;
    }
